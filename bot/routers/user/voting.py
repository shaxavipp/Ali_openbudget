...
async def _finalize_submission(
    ...
    if verifiers_channel_id:
        try:
            vote_number = await settings_repo.get_and_increment_vote_counter(session)
            # Har bir bot o'zining aniq nikini (#hashtag) yuboradi — bu qiymat
            # config.py orqali .env dagi VERIFIER_TAG o'zgaruvchisidan olinadi.
            # Agar VERIFIER_TAG sozlanmagan bo'lsa, "shaxa" standart qiymat sifatida ishlatiladi.
            verifier_tag = getattr(settings, "verifier_tag", "shaxa")
            verifier_caption = (
                f"{vote_number}- #{verifier_tag}\n"
                f"📞 {phone_number}\n"
